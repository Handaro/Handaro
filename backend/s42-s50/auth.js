...
module.exports.verifyAdmin = (req, res, next) => {
	...
	// if the isAdmin property of the req.user is equal to false
		...
		// 403 status code means forbidden. This means that the action that the client is trying to perform is not allowed but not related t their credentials
		return res.status(403).send({
			auth: "Failed",
...
module.exports.errorHandler = (err, req, res, next) => {
	...
	// if the err object has a specific status code, we will store it in the statusCode variable
	// else, we will store 500 as the status code
